...
function ProfileSoundIcon({ size = 60, delay = '0s' }) {
  return (
    <svg width={size} height={size * 0.6} viewBox="0 0 150 90" style={{ overflow: 'visible' }}>
      {/* Голова — простые надёжные фигуры, гарантированно без "дыр" */}
      <circle cx="42" cy="40" r="28" fill={BLUE} />
      <polygon points="65,32 82,40 65,50" fill={BLUE} />
      <polygon points="55,50 68,50 50,62" fill={BLUE} />
      <rect x="20" y="60" width="30" height="30" rx="8" fill={BLUE} />

      {/* Звуковые волны справа, пульсируют */}
      <path className="sound-arc arc-1" d="M95 28 Q108 42 95 56" stroke={INDIGO_BRIGHT} strokeWidth="8" strokeLinecap="round" fill="none" style={{ animationDelay: delay }} />
      <path className="sound-arc arc-2" d="M108 16 Q128 42 108 68" stroke={INDIGO_BRIGHT} strokeWidth="8" strokeLinecap="round" fill="none" style={{ animationDelay: delay, opacity: 0.75 }} />
      <path className="sound-arc arc-3" d="M121 4 Q148 42 121 80" stroke={INDIGO_BRIGHT} strokeWidth="8" strokeLinecap="round" fill="none" style={{ animationDelay: delay, opacity: 0.55 }} />
    </svg>
  );
}
...
export default function App() {
  ...
  async function handleGenerate() {
    ...
    const prompt = quickCheckOnly
      ? `You are a spelling, grammar, and punctuation checker for a non-native English speaker (native language: ${nativeLangLabel}).

Their text: "${inputText}"

Fix ONLY spelling, grammar, and punctuation errors. Do NOT change their wording, tone, or style beyond what's needed to fix actual errors -- keep their voice intact.
List each correction briefly in ${nativeLangLabel}.

Respond ONLY with valid JSON, no markdown, no code fences:
{"rewritten": "...", "changes": ["...", "..."], "toneNote": null, "commonMistakeTip": "..."}`
      : `You help non-native English speakers write professional, natural-sounding English for small business communication.
    ...
    <div className="bg-gradient-animated min-h-screen" style={{ fontFamily: "'Inter', sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10" style={{ position: 'relative', zIndex: 1 }}>
        <div className="flex items-center gap-4 mb-8">
          <div><PersonBubbleMark size={150} /></div>
          <div>
        ...
        <div className="rounded-2xl p-5 mb-5" style={{ background: CARD, border: `1px solid ${LINE}`, backdropFilter: 'blur(16px)' }}>
          ...
          <button
            ...
            className="w-full flex items-center justify-between rounded-lg px-3 py-2.5 mt-4 text-sm"
            style={{
              background: quickCheckOnly ? 'rgba(79,160,255,0.15)' : 'rgba(255,255,255,0.06)',
              border: `1px solid ${quickCheckOnly ? BLUE : LINE}`, color: INK, cursor: 'pointer',
            }}
          >
            <span>\u2705 Quick check only (spelling &amp; grammar, keep my wording)</span>
            <span style={{
              width: 34, height: 18, borderRadius: 999, background: quickCheckOnly ? BLUE : 'rgba(255,255,255,0.15)',
              position: 'relative', flexShrink: 0, transition: 'background 0.15s',
            }}>
              <span style={{
                position: 'absolute', top: 2, left: quickCheckOnly ? 18 : 2, width: 14, height: 14, borderRadius: '50%',
